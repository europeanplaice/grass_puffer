...
const dictionaries = {
  en: {
    ...
    login: {
      ...
      dataStorageItems: [
        'Diary entries are stored only in your own Google Drive (GrassPuffer Diary/ folder)',
        'Sign-in uses OAuth 2.0 with PKCE. Your Google tokens are kept server-side in Cloudflare KV — your browser never receives them',
        'Your browser holds only a session cookie (HttpOnly, Secure). No tokens or diary content are stored in the browser',
      ],
  ...
  ja: {
    ...
    login: {
      ...
      dataStorageItems: [
        '日記は自分の Google Drive のみに保存されます（GrassPuffer Diary/ フォルダ）',
        'ログインには OAuth 2.0（PKCE）を使用。Google のトークンはサーバー側（Cloudflare KV）のみに保持され、ブラウザには渡りません',
        'ブラウザにはセッション Cookie（HttpOnly / Secure）のみが保存されます。トークンや日記の内容はブラウザには保存されません',
      ],
